...
class simulationMP:
    ...
    def start_run(self, run):
        ...
        results = {}
        if self.store.get('s'): results['s'] = self.S[run]
        if self.store.get('pos'): results['pos'] = self.Pos[run]
        if self.store.get('p'): results['p'] = self.P[run]
        if self.store.get('f'): results['f'] = self.F[run]
        if self.store.get('sum_f'): results['sum_f'] = self.sum_F[run]
        results['run'] = self.run
        results['n_steps'] = self.n_steps
        results['n_heads'] = self.n_heads
        if self.mode[run] == 'const': results['v_Coeff'] = self.v_Coeff[0]
        else: results['v_Coeff'] = self.v_Coeff
        results['d'] = self.d[run]
        results['d_t'] = self.d_t[run]
        results['loadF'] = self.loadF[run]
