...
def k_min(s, p, bta, k):
	...
	if abs(s) < (1 + k) / 2:
		return pU * math.exp(-(bta * sSq) / (1 + k)) * math.cosh(bta * s)
...
def k_plus(s, p, d, bta, k, w=False):
	if w: s = wrapping(s, d)
	pU = unitize(p)
	b = np.sqrt(bta)
	c = .5
	#solution of gaussian integral
	gaussianInt = .5 * np.sqrt(math.pi) * ( - special.erf(b * (s - c)) + special.erf(b * (s + c)))
...
def find_neighbours(th, d, bta, k):
	b = 1.
	p = 0
	
	n = 0
	while k_plus(d * (n + .5), p, d, bta, b, w=False) / k_plus(0, p, d, bta, b, w=False) > th:
		n += 1
	return n
...
def k_plus_matrix(s_mat, p, d, bta, k):
	return 	k_plus(s_mat, p, d, bta, k)
